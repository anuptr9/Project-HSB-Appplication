package com.cg.service;

import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.cg.model.UserDestination;
import com.cg.repository.DestinationRepository;
@Service
public class DestinationServiceImpl implements DestinationService {
	
		private DestinationRepository dstRepo;
		
		@Autowired
		public DestinationServiceImpl(DestinationRepository dstRepo) {
			this.dstRepo=dstRepo;
			
		}

		@Override
		public UserDestination addDestination(UserDestination admdst) {
			UserDestination savedDestination=dstRepo.save(admdst);
			return savedDestination;
		}
		
		@Override
		public List<UserDestination> getAllDestinations() {
			return (List<UserDestination>)dstRepo.findAll();
		}

		@Override
		public void deleteDestinationByDstId(int dstId) {
			dstRepo.deleteById(dstId);
		}

		@Override
		public UserDestination updateDestination(UserDestination admdst) {
			UserDestination dstRecordUpdate;
			Optional<UserDestination> opt=dstRepo.findById(admdst.getDstId());
			if(opt.isPresent())
			{
				dstRecordUpdate=opt.get();
				dstRecordUpdate.setDstName(admdst.getDstName());
				dstRepo.save(dstRecordUpdate);
			}
			else
			{
				return new UserDestination();
			}
			return dstRecordUpdate;
		}

}

